...
if ! env -u GITHUB_TOKEN gh repo view LeeDoan/301w53 &>/dev/null; then
    echo "Creating public repository LeeDoan/301w53 on GitHub..."
    env -u GITHUB_TOKEN gh repo create 301w53 --public --confirm
else
    echo "Repository LeeDoan/301w53 already exists on GitHub."
fi

# 3. Configure local git remotes
if ! git remote | grep -q "^origin$"; then
    echo "Adding git remote origin..."
    git remote add origin https://github.com/LeeDoan/301w53.git
else
    echo "Remote origin already configured."
    # Ensure correct remote URL
    git remote set-url origin https://github.com/LeeDoan/301w53.git
fi
...
env -u GITHUB_TOKEN gh api repos/LeeDoan/301w53/pages \
  -X POST \
...
echo "Live URL: https://leedoan.github.io/301w53/"
